...
npx tsc --module ESNext --target ES2022 --moduleResolution NodeNext --esModuleInterop --skipLibCheck server/simple-worker.ts --outDir dist/server
...
if grep -q "export default" test-worker-module.js; then
